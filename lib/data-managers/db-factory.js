...
class DbFactory {
  getSupportedDatabases() {
    return [
      ...
      {name: 'Redshift', prefix: 'postgresql', port: 5439}
    ];
  }
  ...
  file() {
    let filename = 'data-ace-connections.cson';
  ...
  createDataManagerForUrl(url) {
    ...
    switch (protocol) {
      ...
      case 'redshift': {
        var RedshiftManager = require('./redshift-manager');
        return new RedshiftManager(url);
      }
      case 'sqlserver': {
