...
import DetailsHeaderView from './details-header-view'
...
export default class DetailsView {
  ...
  getPosition() {
    return atom.config.get('data-ace.showDetailsViewOnRightSide') ? 'right' : 'left'
  }

  createView() {
    this.element = document.createElement('section')
    this.element.className = 'data-ace data-ace-panel data-details-panel tool-panel panel panel-left padding'

    this.resizeHandle = document.createElement('div')
    this.resizeHandle.classList.add('resize-handle')
    this.element.appendChild(this.resizeHandle)

    this.detailsHeaderView = new DetailsHeaderView(false)
    this.element.appendChild(this.detailsHeaderView.element)
  ...
  clearDbManager() {
    ...
    while(this.dbList.firstChild) {
      this.dbList.removeChild(this.dbList.firstChild)
    }
  }
  ...
  setState(connection, dbNames, selectedDb, useEditorAsQuery) {
    this.detailsHeaderView.setConnection(connection)
    this.detailsHeaderView.addDbNames(dbNames)
    this.detailsHeaderView.setSelectedDatabaseName(selectedDb)
    //this.useEditorAsQuerySource(useEditorAsQuery)
  }

  addConnection(connectionName) {
    this.detailsHeaderView.addConnection(connectionName)
  }
  ...
  getSelectedConnection() {
    return this.detailsHeaderView.getSelectedConnection()
  }

  getSelectedDatabase() {
    return this.detailsHeaderView.getSelectedDatabase()
  }

  getSelectedSchema() {
    return this.detailsHeaderView.getSelectedSchema()
  }

  clearDatabaseSelection() {
    this.detailsHeaderView.clearDatabaseSelection()
  }

  setDatabaseSelection(dbNames, selectedName) {
    this.detailsHeaderView.addDbNames(dbNames)
    this.detailsHeaderView.setSelectedDatabaseName(selectedName)
  }

  setSchemaSelection(schemaNames, selectedSchemaName) {
    this.detailsHeaderView.addSchemaNames(schemaNames)
    this.detailsHeaderView.setSelectedSchemaName(selectedSchemaName)
  }

  // register for selected connection change events
  onConnectionChanged(onConnectionChangedFunc) {
    return this.detailsHeaderView.onConnectionChanged(onConnectionChangedFunc)
  }

  // Register for selected database change events
  onDatabaseChanged(onDatabaseChangedFunc) {
    return this.detailsHeaderView.onDatabaseChanged(onDatabaseChangedFunc)
  }

  onSchemaChanged(onSchemaChangedFunc) {
    return this.detailsHeaderView.onSchemaChanged(onSchemaChangedFunc)
  }

  onQueryCancel(onQueryCancelFunc) {
    return this.detailsHeaderView.onQueryCancel(onQueryCancelFunc)
  }

  onDisconnect(onFunc) {
    return this.detailsHeaderView.onDisconnect(onFunc)
  }
