...
export default class DataAceController {
  ...
  queryTable(el) {
    ...
    let query = this.getDataManager(currentViewState.connectionName).getTableQuery(tableName)
    let selectedLimit = this.mainView.getSelectedLimit()
    if (selectedLimit !== 'none') {
      query += ' LIMIT ' + selectedLimit + ';'
    }
    let database = this.detailsView.getSelectedDatabase()
    query += `SELECT ordinal_position AS "Position", column_name AS "Column", column_type AS "Type", column_default As "Default", is_nullable AS "Nullable", character_set_name AS "Character Set", collation_name AS "Collation", privileges AS "Privileges" from information_schema.columns where table_schema = '` + database + `' AND table_name = '` + tableName + `' ORDER BY ordinal_position`
