...
export default class DataAceController {
  constructor(serializeState, statusBarManager) {
    ...
    this.detailsView.onSchemaChanged(() => this.onSchemaChanged())
    ...
    atom.commands.add('atom-workspace', 'data-ace:toggle-results-view', () => this.toggleMainView())
    atom.commands.add('atom-workspace', 'data-ace:toggle-details-view', () =>  this.toggleDetailsView())
    ...
    atom.commands.add('atom-workspace', 'data-ace:describe-table', (el) => this.describeTable(el))
    atom.commands.add('atom-workspace', 'data-ace:generate-create-statement', (el) => this.generateCreateStatement(el))
  ...
  onDatabaseChanged() {
    let selectedDatabase = this.detailsView.getSelectedDatabase()
    let currentViewState = this.getOrCreateCurrentResultView()
    currentViewState.database = selectedDatabase

    let newManager = this.getDataManager(this.detailsView.getSelectedConnection())
    this.setupAutocomplete(currentViewState.database, newManager)

    newManager.getSchemaNames(selectedDatabase).then(schemaNameArray => this.detailsView.setSchemaSelection(schemaNameArray, currentViewState.schema))
  }

  onSchemaChanged() {
    let selectedSchema = this.detailsView.getSelectedSchema()
    let currentViewState = this.getOrCreateCurrentResultView()
    currentViewState.schema = selectedSchema
  }
  ...
  queryTable(el) {
    ...
    let currentViewState = this.getOrCreateCurrentResultView()
    let tableName = el.target.innerText
    let schemaName = currentViewState.schema
    let query = this.getDataManager(currentViewState.connectionName).getTableQuery(tableName, schemaName)
  ...
  describeTable(el) {
    ...
    let tableName = el.target.innerText
    let schemaName = currentViewState.schema
    let query = this.getDataManager(currentViewState.connectionName).getTableDescription(tableName, schemaName)
    this.actuallyExecute(currentViewState, query)
  }

  generateCreateStatement(el) {
    if (this.isEditorNotActive()) {
      return
    }
    ...
    let tableName = el.target.innerText
    let schemaName = currentViewState.schema
    let query = this.getDataManager(currentViewState.connectionName).getCreateStatement(tableName, schemaName)
    this.actuallyExecute(currentViewState, query)
  }
  ...
  execute() {
    ...
    let selectedLimit = this.mainView.getSelectedLimit()
    if (selectedLimit !== 'none') {
      query = 'SELECT * FROM (' + query + ') AS Result LIMIT ' + selectedLimit
    }
  ...
  actuallyExecute(executingViewState, query) {
    ...
    this.getDataManager(executingViewState.connectionName)
      .execute(executingViewState.database, query, queryToken => {
        executingViewState.queryToken = queryToken
      })
      .then(results => {
