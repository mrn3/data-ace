...
export default class DataAceController {
  ...
  queryTable(el) {
    ...
    let query = this.getDataManager(currentViewState.connectionName).getTableQuery(el.target.innerText)
    let selectedLimit = this.mainView.getSelectedLimit()
    if (selectedLimit !== 'none') {
      query += ' LIMIT ' + selectedLimit + ';'
    }
    query += "select * from information_schema.columns where table_name = 'role'"
