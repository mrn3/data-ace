...
class SQLMetaProvider {
  ...
  getTableNames(editor, tablePrefix, schema) {
    let tables = editor.dataAceTables || [];
    ...
    return valid.map((table) => {
      return {
        ...
        rightLabelHTML: `<span class="data-ace autocomplete autocomplete-tbl"></span>${table.type}`
      };
    });
  }

  getColumnNames(editor, columnPrefix, objectName) {
    let columns = editor.dataAceColumns || [];
    ...
    return valid.map((col) => {
      return {
        text: col.name,
        rightLabelHTML: '<span class="data-ace autocomplete autocomplete-col"></span>Column',
