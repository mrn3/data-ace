...
describe('InsertMetaProvider', () => {
  ...
  let editor = {
    ...
    it('filters by table name including the ALL suggestion when no previous columns exist', () => {
        ...
        expect(cols).toContain({text: 'test_col', rightLabelHTML: '<span class="data-ace autocomplete autocomplete-col"></span>Column', leftLabel: 'varchar'})
    ...
    it('filters by table name and only includes columns that are not existing already', () => {
        ...
        expect(cols).toContain({text: 'other_col', rightLabelHTML: '<span class="data-ace autocomplete autocomplete-col"></span>Column', leftLabel: 'bool'})
...
describe('SQLMetaProvider', () => {
  describe('Get table names', () => {
    it('filters by prefix', () => {
      ...
      expect(tables).not.toContain({text: 'other_table', rightLabelHTML: '<span class="data-ace autocomplete autocomplete-tbl"></span>Table'})
      expect(tables).toContain({text: 'test_table', rightLabelHTML: '<span class="data-ace autocomplete autocomplete-tbl"></span>Table'})
      expect(tables).toContain({text: 'test_table_2', rightLabelHTML: '<span class="data-ace autocomplete autocomplete-tbl"></span>Table'})
