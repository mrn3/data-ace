...
function findDataAceDialog(workspace) {
  return [].slice.call(workspace.getElementsByTagName('section')).filter(i => i.classList.contains('data-ace') && i.classList.contains('dialog'));
}
...
describe('NewConnectionDialog', () => {
  ...
  describe('when calling show and close', () => {
    it("shows then closes", () => {
      ...
      expect(findDataAceDialog(document).length).toEqual(0);
      view.show();
      expect(findDataAceDialog(document).length).toEqual(1);
      view.close();
      expect(findDataAceDialog(document).length).toEqual(0);
    });
  });
